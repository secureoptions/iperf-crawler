...
def lambda_handler(event, context):
	...
	def iperf_crawler(ec2):
		...
		try:
			...
			for item in get_group_subs['Items']:
				...
				account_id = sdb.get_attributes(
					DomainName='iperf-crawler',
					ItemName=subnet_id,
					AttributeNames=[
						'Account'
					])
				if account_id['Attributes'][0]['Value'] == account:
					group_id = sdb.get_attributes(
						DomainName='iperf-crawler',
						ItemName=subnet_id,
						AttributeNames=[
							'Group'
						])
					group_id = group_id['Attributes'][0]['Value']
		...
		for region in response['Regions']:
			if account != parent_account:
				assume_role = sts.assume_role(
						RoleArn='arn:aws:iam::%s:role/IperfCrawlerLambdaRole' % account,
				...
				for tag in subnet['Tags']:
					if re.match(key_regex, tag['Key']) and re.match(value_regex, tag['Value']):
						...
						try:
							...
							# determine if a secondary account number is needing to be included with this subnet worker's metadata
							if parent_account == account:
								registered_account = 'parent'
							else:
						...
						try:
							...
							if len(get_group_subs['Items']) == 2:				
								...
								for item in get_group_subs['Items']:
									...
									account_number = sdb.get_attributes(
															DomainName='iperf-crawler',
															ItemName=item['Name'],
															AttributeNames=['Account']
															)
									
									EC2_REGION = subnet_attributes['Attributes'][0]['Value']
									if account_number['Attributes'][0]['Value'] == 'parent':
										ec2 = boto3.client('ec2', region_name = EC2_REGION)
									else:
										assume_role = sts.assume_role(
											RoleArn='arn:aws:iam::%s:role/IperfCrawlerLambdaRole' % account_number['Attributes'][0]['Value'],
											RoleSessionName='deploy_iperf_worker'
											)
										ec2 = boto3.client('ec2', 
												aws_access_key_id=assume_role['Credentials']['AccessKeyId'],
									...
									launch_instance = ec2.run_instances(
														...
														UserData = ('#!/bin/bash\n'
																   ...
																   'echo PARENT_ACCOUNT="\'%s\'\" >> /home/ec2-user/vars.py\n'
																   'python /home/ec2-user/worker*') % 
																   (URLS[worker_selector], state_machine['stateMachineArn'], REGION, EC2_REGION, workerA_ARN, workerB_ARN, sg_id, IPERF_FLAGS, MTR_FLAGS, ','.join(subnets),tag['Value'], parent_account),

														MinCount=1,
														MaxCount=1,
														IamInstanceProfile={'Name':'IperfCrawlerEc2'},
														)
		...
		# code snippet to set necessary assume role creds					
		def set_creds(x,y):
	...
	# Determine if there is more than this account that needs to polled
	ACCOUNTS = ACCOUNTS.replace(' ','')
	ACCOUNTS = ACCOUNTS.split(',')
	parent_account = sts.get_caller_identity()
	parent_account = parent_account['Account']
	
	if len(ACCOUNTS) > 1 or ACCOUNTS[0] != '000000000000':
